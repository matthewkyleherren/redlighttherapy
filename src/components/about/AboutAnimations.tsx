...
function createAboutValuesCardStack(libs: AnimationLibs) {
  const container = document.getElementById('aboutValues');
  const cardNodes = container?.querySelectorAll<HTMLElement>('.value');
  const leftBtn = document.getElementById('aboutValuesArrowLeft') as HTMLButtonElement | null;
  const rightBtn = document.getElementById('aboutValuesArrowRight') as HTMLButtonElement | null;
...
function createAboutSection1(libs: AnimationLibs): TimelineSection {
  const section = document.getElementById('aboutSection1');
  const header = document.getElementById('aboutHeader1');
  const title = document.getElementById('aboutHeader1Title');
  const description = document.getElementById('aboutHeader1Description');
  const link = document.getElementById('aboutHeader1Link');

  if (!section || !header || !title || !description || !link) {
    throw new Error('About section 1 elements not found');
  }
...
function createAboutPhilosophySection(libs: AnimationLibs): TimelineSection {
  const section = document.querySelector<HTMLElement>('.aboutPhilosophySection');
  const valuesWrapper = document.getElementById('aboutValuesWrapper');

  if (!section || !valuesWrapper) {
    throw new Error('About philosophy section elements not found');
  }

  libs.gsap.set(valuesWrapper, { opacity: 0, y: 40 });

  let cardStack: ReturnType<typeof createAboutValuesCardStack> = null;
    ...
    animations: [
      {
        ...
        update: (progress) => {
          if (progress > 0.1) {
            ...
            if (!cardStack) {
              cardStack = createAboutValuesCardStack(libs);
            }
          }
        },
      },
...
function playAboutSection1Entrance(libs: AnimationLibs) {
  const title = document.getElementById('aboutHeader1Title');
  const description = document.getElementById('aboutHeader1Description');
  const link = document.getElementById('aboutHeader1Link');
...
function setupTimeline(libs: AnimationLibs): Timeline {
  ...
  timeline.initSection(createAboutSection1(libs));
  timeline.initSection(createAboutPhilosophySection(libs));
  timeline.refresh();
  return timeline;
}
...
export function AboutAnimations() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      playAboutSection1Entrance(libs);
    }
